...
EXPECTED_ADAPTER_VERSION = "2026.07.21.2"
if getattr(data_loader, "ADAPTER_VERSION", None) != EXPECTED_ADAPTER_VERSION:
...
def load_reference_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    factors = data_loader.load_emission_factors(DATA_DIR / "emission_factors.csv")
    sample = data_loader.load_activity_data(DATA_DIR / "sample_activity_data.csv")
...
def prepare_uploaded_data(uploaded_file, sample: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    ...
    return data_loader.load_activity_data(uploaded_file, uploaded_file.name), uploaded_file.name
...
with st.sidebar:
    ...
    template_bytes = data_loader.build_excel_template(sample_data.head(3))
    demo_bytes = data_loader.build_excel_template(sample_data)
...
try:
    ...
    raw_data = data_loader.ensure_activity_adapted(raw_data)
except Exception as exc:
    st.error(f"文件读取失败：{exc}")
    st.stop()

with st.sidebar:
    mapping_status = raw_data.get("mapping_status")
    mapped_count = int(mapping_status.isin(["自动识别", "需复核"]).sum()) if mapping_status is not None else 0
    source_details = [f"{len(raw_data):,} 条活动记录"]
    if mapped_count:
        source_details.append(f"自动识别 {mapped_count} 条")
    if raw_data.attrs.get("source_sheet"):
        source_details.append(f"工作表 {raw_data.attrs['source_sheet']}")
    if raw_data.attrs.get("header_row", 1) > 1:
        source_details.append(f"表头第 {raw_data.attrs['header_row']} 行")
    source_details.append(f"识别器 {EXPECTED_ADAPTER_VERSION}")
